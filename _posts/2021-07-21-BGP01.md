...
5.建立邻居关系：  
	...
	* 如果设备在边界上（一段建立IBGP，另一端建立EBGP，因为外部路由条目引入的时候传到端口IP上，而IBGP之间用环回口建立邻居，所以无法把外部路由引入到域内各个设备上，所以需要建立一个到对端设备环回口的路由条目）：`peer 对端环回口 next-hop-local`。  
6.在IBGP内部各设备的环回口必须互通，互通可以用静态路由、动态路由协议等等方式，虽然有的设备是直连，但是环回口并没有距离的概念，所以必须建立环回口的路由条目，使得各个设备的环回口可以互通。  
7.区域之间的直连需要引入：`import-route direct`。  
...
[pt_01]: https://gitee.com/nora2nan/blog-image/raw/master/55_BGP01/01.png
