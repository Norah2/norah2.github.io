...
4. 配置IS-IS协议：首先宣告区域号，再在各个接口上打开isi
   
5. s功能，其中拓扑图中是AR1和AR2只在和AR3连接的端口上运行ISIS协议，并且AR3和AR4之间也需要运行ISIS协议；宣告完成之后AR1、AR2、AR3之间能互通，AR4、AR5、AR6之间能够互通；**需要注意运行ISIS协议设备的环回口上也要开启isis**，否则各个设备之间的环回口不能互通，这样会使得BGP邻居之间的路由条目无法传递；**配置isis的时候要养成设置设备类型的习惯，否则里面会在相同的设备上建立多个邻居，不设置类型的话也不会有太大的影响**  
...
AR7上通过静态路由到达AR1/2；  
AR1/2到达AR3是ISIS协议学习到的路由条目；  
AR3到AR4是通过EBGP邻居传递过去的；  
AR4到AR5/6同样是ISIS动态协议学习到的；
...
[pt_01]: https://gitee.com/nora2nan/blog-image/raw/master/57_BGP03/01.png
